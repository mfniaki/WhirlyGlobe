...
"void main()\n"
"{\n"
"   float ndotl = max(0.0, dot(v_adjNorm, light[0].direction));\n"
"   ndotl = pow(ndotl,0.25);\n"
"   float v_sunComp = ndotl;\n"
"   vec4 ambient = light[0].ambient;\n"
"   vec4 diffuse = ndotl * light[0].diffuse;\n"
"   vec4 color;\n"
"   if (u_numLights > 0)\n"
"     color = vec4(ambient.xyz * material.ambient.xyz * v_color.xyz + diffuse.xyz * v_color.xyz,v_color.a) * u_fade;\n"
"   else\n"
"     color = v_color * u_fade;\n"
"\n"
"  vec4 baseColor0 = texture2D(s_baseMap0, v_texCoord0);\n"
"  vec4 baseColor1 = texture2D(s_baseMap1, v_texCoord1);\n"
"  gl_FragColor = mix(baseColor0,baseColor1,1.0-v_sunComp);\n"
"}\n"
;
...
void SetupDefaultShaders(Scene *scene)
{
    ...
    {
        NSLog(@"SetupDefaultShaders: Default triangle and line shaders didn't compile.  Nothing will work.");
        delete triShader;
        delete lineShader;
    } else {
